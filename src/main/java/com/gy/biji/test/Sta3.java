    ...
    public static void main(String[]args) throws Exception {
    	...
    	for(int K=3;K<=9;K++) {
	    	...
	    	for(int M=0;M<total;M++) {
		    	...
		    	if(getType(pll.get(pll.size()-2)) == 5) Constance.san_ge_tou_num++;
		    	if(getType(pll.get(pll.size()-2)) == 4) Constance.shun_qing_num++;
		    	if(getType(pll.get(pll.size()-2)) == 3) Constance.jin_hua_num++;
		    	if(getType(pll.get(pll.size()-2)) == 2) Constance.tuo_la_ji_num++;
		    	if(getType(pll.get(pll.size()-2)) == 1) Constance.dui_zi_num++;
		    	if(getType(pll.get(pll.size()-2)) == 0) Constance.dan_zhi_zi_num++;
	    	...
	    	str += "最大的两家牌  >= "+Constance.typeStr[5]+":"+ df.format((double)Constance.san_ge_tou_num*(double)100 / (double)(total)) + "%\n";
	    	str += "最大的两家牌  >= "+Constance.typeStr[4]+":"+ df.format((double)Constance.shun_qing_num*(double)100 / (double)(total)) + "%\n";
	    	str += "最大的两家牌  >= "+Constance.typeStr[3]+":"+ df.format((double)Constance.jin_hua_num*(double)100 / (double)(total)) + "%\n";
	    	str += "最大的两家牌  >= "+Constance.typeStr[2]+":"+ df.format((double)Constance.tuo_la_ji_num*(double)100 / (double)(total)) + "%\n";
	    	str += "最大的两家牌  >= "+Constance.typeStr[1]+":"+ df.format((double)Constance.dui_zi_num*(double)100 / (double)(total)) + "%\n";
	    	str += "最大的两家牌  >= "+Constance.typeStr[0]+":"+ df.format((double)Constance.dan_zhi_zi_num*(double)100 / (double)(total)) + "%\n";
